...
#! @Description
#!  Given two representations  <A>M</A>  and  <A>N</A>  over a finite dimensional quotient
#!  of a path algebra, this function computes a basis of the homomorphisms
#!  that factor through a projective representation.
#! @Returns inclusion into <Ref Filt="IsHomSpace"/>
#! @Arguments M, N
DeclareOperation( "HomFactoringThroughProj", [ IsQuiverRepresentation, IsQuiverRepresentation ] );
...
#! @Description
#!  Given a representation  <A>R</A>  over a finite dimensional quotient of a
#!  path algebra, this function computes the homomorphism from
#!  <C>End(R) ----> End(R)/P(R,R)</C>, where <C>P(R,R)</C> is the ideal in <C>End(R)</C>
#!  generated by the morphisms factoring through projective modules.
#!  If  <A>R</A>  is the zero representation, then an empty list is returned.
#! @Returns <C>IsAlgebraHomomorphism</C>
#! @Arguments R
DeclareOperation( "StableEndomorphismAlgebraModuloProj", [ IsQuiverRepresentation ] );

#! @Description
#!  Given subobjects <M>M_i \to X</M>  for <M>i = 1,\ldots,n</M> of an object  <M>X</M>  by
#!  <M>n</M>  monomorphism  <M>f_i</M>, this function computes the sum of the images
#!  of  <M>f_i</M>  in the sense that it computes the subobject <M>\sum_i M_i</M>
#!  generated by the images of  <M>f_i</M>.  It returns the maps 
#!  <M>\sum_i M_i ---> X</M>,  and <M>M_i ---> \sum_i M_i</M>.
#! @Returns a list of <Ref Filt="IsFieldCategoryMorphism"/>
#! @Arguments list
DeclareOperation( "SumOfSubobjects", [ IsDenseList ] );
